...
final class WorkspaceIndex {
    ...
    convenience init(policy: WorkspaceIndexPolicy = .default) {
        self.init(policy: policy, enumerator: Self.defaultEnumerator, reader: Self.defaultReader)
    }

    init(policy: WorkspaceIndexPolicy, enumerator: @escaping Enumerator, reader: @escaping Reader) {
        self.policy = policy
    ...
    nonisolated private static func buildSnapshot(
        ...
        enumerator: @escaping Enumerator,
        reader: @escaping Reader,
        cancellationToken: WorkspaceCancellationToken
    ) -> WorkspaceIndexSnapshot {
        let urls = enumerator(rootURL, policy)
            .filter { policy.accepts($0, isDirectory: false) }
            .prefix(policy.maximumFiles)
        ...
        DispatchQueue.concurrentPerform(iterations: min(policy.readConcurrency, max(1, urls.count))) { _ in
            while true {
                ...
                guard let result = reader(url), result.byteCount <= policy.maximumBytesPerFile else {
                    lock.lock(); skipped += 1; lock.unlock()
                    continue
                }
        ...
        return WorkspaceIndexSnapshot(rootURL: rootURL, revision: revision, entries: entries, skippedFiles: skipped)
    }
    ...
    private static let defaultEnumerator: Enumerator = { root, policy in
        ...
        for case let url as URL in iterator {
            ...
            if values?.isRegularFile == true, policy.accepts(url, isDirectory: false) { urls.append(url) }
        }
        return urls
    }
